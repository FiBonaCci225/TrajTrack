...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if query.dim() == 3:
            query = query.contiguous().view(batch_size, trg_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
            key = key.contiguous().view(batch_size, src_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
            value = value.contiguous().view(batch_size, src_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
            # attn_mask = attn_mask.unsqueeze(1).repeat(1, self.num_heads, 1, 1)
        elif query.dim() == 4:
            nsamp = query.size(2)
            query = query.view(batch_size, trg_len, nsamp, self.num_heads, self.head_dim).permute(0, 2, 3, 1, 4).contiguous()
            key = key.view(batch_size, src_len, nsamp, self.num_heads, self.head_dim).permute(0, 2, 3, 1, 4).contiguous()
            value = value.view(batch_size, src_len, nsamp, self.num_heads, self.head_dim).permute(0, 2, 3, 1, 4).contiguous()
            # attn_mask = attn_mask.unsqueeze(1).unsqueeze(1).repeat(1, nsamp, self.num_heads, 1, 1)
            # query = query.contiguous().view(batch_size, self.num_heads * nsamp, trg_len, self.head_dim)
            # key = key.contiguous().view(batch_size, self.num_heads * nsamp, src_len, self.head_dim)
            # value = value.contiguous().view(batch_size, self.num_heads * nsamp, src_len, self.head_dim)

        # Scaled dot-product attention
        attn_output = F.scaled_dot_product_attention(query, key, value).contiguous()
        # dropout = torch.nn.Dropout(p=0.1)  # 设置 dropout 概率为 10%
        # attn_output = dropout(attn_output).contiguous()
        ...
        if query.dim() == 4:
            ...
            attn_output = attn_output.view(batch_size, attn_output.size(1), attn_output.size(2), self.embed_dim)
...
def agent_aware_attention(query: Tensor,
    ...
    if in_proj_weight_self is not None:
        ...
        attn_weight_self_mask = attn_weight_self_mask.repeat(
            [attn_output_weights.shape[1] // num_agent, attn_output_weights.shape[2] // num_agent]).unsqueeze(0)
        ...
        # attn_output_weights = attn_output_weights_inter * (1 - attn_weight_self_mask) + attn_output_weights_self * attn_weight_self_mask
        attn_output_weights = attn_output_weights_inter * attn_weight_self_mask
        if attn_mask is not None:
            if attn_mask.dtype == torch.bool:
                attn_output_weights.masked_fill_(attn_mask, float('-inf'))
            else:
...
class TrajFormerEncoderLayer(Module):
...
class TrajFormerDecoderLayer(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, self_mask: Optional[Tensor] = None, cross_mask: Optional[Tensor] = None,
        ...
        Shape:
            see the docs in Transformer class.
        """
        tgt2 = self.self_attn(tgt, tgt, tgt, attn_mask=self_mask,)
                              # key_padding_mask=tgt_key_padding_mask, num_agent=num_agent, need_weights=need_weights)
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)
        tgt2 = self.multihead_attn(tgt, memory, memory, attn_mask=cross_mask,)
...
class TrajFormerEncoder(Module):
...
class TrajFormerDecoder(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, self_mask: Optional[Tensor] = None,
                cross_mask: Optional[Tensor] = None, tgt_key_padding_mask: Optional[Tensor] = None,
        ...
        for i, mod in enumerate(self.layers):
            output = mod(output, memory, self_mask=self_mask,
