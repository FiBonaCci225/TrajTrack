...
class ResVoxelNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.voxel_encoder = DynamicSimpleVFE(
            point_cloud_range=point_cloud_range,
            voxel_size=voxel_size)
        self.backbone = VoxelResBackBone8x({}, points_features, grid_size)
        ...
        # )
        # self.intens_encoder = SparseEncoder(
        #     in_channels=1,
        #     sparse_shape=grid_size,
        #     norm_cfg=dict(type='SyncBN', eps=1e-3, momentum=0.01),
        #     base_channels=16,
        ...
        #     block_type='conv_module',
        # )
        # self.fuse = nn.Sequential(MLP(256, 128))
        # self.mlp = nn.Sequential(nn.Conv1d(1, 64, kernel_size=1, bias=False),
        #                          nn.BatchNorm1d(64),
        #                          nn.ReLU(),
    ...
    def voxelize(self, points):
        ...
        intens = torch.cat(intens, dim=0)

        return voxels, coors, intens

    def forward(self, x, batch_size=0):
        feats, coords, intens = self.voxelize(x)
        voxel_features, coords = self.voxel_encoder(feats, coords)
        # intens_weight = torch.sigmoid(self.mlp(voxel_features[:, 3:].permute(1, 0).unsqueeze(0))).squeeze(0).permute(1, 0)
        # voxel_features = voxel_features[:, :3] * intens_weight
        if batch_size == 0:
            batch_size = coords[-1, 0] + 1

        encoder_features = self.backbone(voxel_features, coords, batch_size)
        #
        # intens_features = self.intens_encoder(voxel_features[:,3:], coords, batch_size)encoder_features = self.middle_encoder(voxel_features, coords, batch_size)
        # encoder_features = self.fuse(torch.cat((encoder_features, intens_features), dim=1))
