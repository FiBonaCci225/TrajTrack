...
class ContextEncoder(nn.Module):
    def __init__(self, cfg, ctx, **kwargs):
        ...
        # self.reduct=nn.Linear(1024, self.model_dim // 2)
        # self.fuse=nn.Linear(self.model_dim + self.model_dim // 2, self.model_dim)
    ...
    def forward(self, data):
        ...
        # h = self.fuse(torch.cat((torch.mean(data['context_enc'], dim=1), self.reduct(data['motion'])), dim=-1))
        if self.pooling == 'mean':
            # data['agent_context'] = torch.mean(data['context_enc'], dim=1)
            data['agent_context'] = torch.mean(tf_in_pos, dim=1)
        else:
...
class FutureEncoder(nn.Module):
    def __init__(self, cfg, ctx, **kwargs):
        ...
        # self.reduct=nn.Linear(1024, self.model_dim // 2)
        self.fuse=nn.Linear(self.model_dim + self.model_dim, self.model_dim)
    ...
    def forward(self, data, reparam=True):
        ...
        # self_mask = data['fut_mask'].to(torch.bool).unsqueeze(2) & data['fut_mask'].to(torch.bool).unsqueeze(1)
        # cross_mask = data['fut_mask'].to(torch.bool).unsqueeze(2) & data['pre_mask'].to(torch.bool).unsqueeze(1)
        # tf_in_pos = tf_in_pos.view(-1, traj_in.shape[0], self.model_dim)
        # tf_out = self.tf_decoder(tf_in_pos, data['context_enc'], #self_mask=self_mask, cross_mask=cross_mask,
        #                             num_agent=data['agent_num'])#.transpose(0,1)
        # tf_out = tf_out.view(traj_in.shape[0], -1, self.model_dim)

        if self.pooling == 'mean':
            h = torch.mean(tf_in_pos, dim=1)
        else:
            h = torch.max(tf_out, dim=1)[0]
        if self.out_mlp_dim is not None:
            h = self.out_mlp(h)
        # h = self.fuse(torch.cat((h,self.reduct(data['motion'])), dim=-1))
        h = self.fuse(torch.cat((h, data['agent_context']), dim=-1))
...
class FutureDecoder(nn.Module):
    ...
    def decode_traj_ar(self, data, mode, context, pre_motion, pre_vel, pre_motion_scene_norm, z, sample_num,
        ...
        for i in range(self.future_frames):
            ...
            con = context.unsqueeze(1).repeat_interleave(tf_in_pos.size(1), dim=1)
            tf_out = self.fuse(torch.cat((tf_in_pos, con), dim=-1))
    ...
    def forward(self, data, mode, sample_num=1, autoregress=True, z=None, need_weights=False):
        ...
        context = data['agent_context'].unsqueeze(-2).repeat_interleave(sample_num, dim=-2)
