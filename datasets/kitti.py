    ...
    def get_frames(self, seq_id, frame_ids):
        if self.preloading:
            ...
            if len(frame_ids) == 2:
                if frame_ids[0] >= 6:
                    offset = 3
                    cur_id = frame_ids[0] - offset
                elif frame_ids[0] >= 3:
                    offset = frame_ids[0] - 3
                    cur_id = 3
                elif frame_ids[0] >= 0:
                    offset = 0
                    cur_id = frame_ids[0]
                traj_id = list(range(cur_id - 3, cur_id + 1)) + list(range(cur_id + 1, cur_id + 12 + 1))
                coor = []
                mask = []
                heading = seq_annos[frame_ids[0]]['rotation_y']
                for idx in traj_id:
                    if idx < 0:
                        coor.append([seq_annos[0][-4] ,seq_annos[0][-3]])
                        mask.append(0)
                    elif idx >= len(seq_annos):
                        coor.append([seq_annos[-1][-4] ,seq_annos[-1][-3]])
                        mask.append(0)
                    else:
                        xy = [seq_annos[idx][-4] ,seq_annos[idx][-3]]
                        coor.append(xy)
                        mask.append(1)
