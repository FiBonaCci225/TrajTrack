...
class TrainSamplerTraj(torch.utils.data.Dataset):
    ...
    def processing(data, config):
        ...
        # ================================================================================
        past_nums = 3
        pre_motion_3D = data['traj']['coor'][:(past_nums+1)]
        fut_motion_3D = data['traj']['coor'][(past_nums+1):]
        pre_motion_mask = data['traj']['mask'][:(past_nums+1)]
        fut_motion_mask = data['traj']['mask'][(past_nums+1):]
        heading = data['traj']['rot']
        offset = data['traj']['offset']
        traj = {'pre_motion_3D': torch.as_tensor(pre_motion_3D, dtype=torch.float32),
        'fut_motion_3D': torch.as_tensor(fut_motion_3D, dtype=torch.float32),
        'pre_motion_mask': torch.as_tensor(pre_motion_mask, dtype=torch.bool),
        'fut_motion_mask': torch.as_tensor(fut_motion_mask, dtype=torch.bool),
        'heading': torch.as_tensor(heading, dtype=torch.float32),
        'offset': torch.as_tensor(offset, dtype=torch.int)}
        # ================================================================================
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            (prev_frame, this_frame), traj = self.dataset.get_frames(tracklet_id, frame_ids=frame_ids)
            data = {
...
class TestSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return self.dataset.get_frames(index, frame_ids)[0]
