...
class NuScenesDatasetTraj:
    ...
    def _build_for_agentformer(self):
        index = 0
        for instance in self.track_instances:
            if instance['nbr_annotations'] < 14:
                continue
            track_anno = []
            ...
            while curr_anno_token != '':
                ...
                sample_data_lidar['scene_name'] = self.nusc.get('scene', sample['scene_token'])['name']

                data = np.ones(18) * -1.0
                data[0] = frame_id
                data[1] = 0
                data[10] = ann_record['size'][0]
                data[11] = ann_record['size'][2]
                data[12] = ann_record['size'][1]
                data[13] = ann_record['translation'][0]
                data[14] = ann_record['translation'][2]
                data[15] = ann_record['translation'][1]
                data[16] = Quaternion(ann_record['rotation']).yaw_pitch_roll[0]
            ...
            # np.savetxt(f'{self.path}/label/{self.split}/traj_{index}.txt', cvt_data, fmt='%s')
            index += 1
            if index == 2120:
                a = 1
    ...
    def _build_tracklet_anno(self):
        ...
        index = 0
        for instance in self.track_instances:
            if instance['token'] == 'be7f82a89e2d408ea728de97f0cc6d4c':
                a = 1
            track_anno = []
            curr_anno_token = instance['first_annotation_token']
            frame_id = 0
    ...
    def get_frames(self, seq_id, frame_ids):
        if self.preloading:
            ...
            # ================================================================================
            if len(frame_ids) == 2:
                if frame_ids[1] >= offset_def + past_nums:
                    offset = offset_def
                    cur_id = frame_ids[1] - offset
                elif frame_ids[1] >= past_nums:
                    offset = frame_ids[1] - past_nums
                    cur_id = past_nums
                elif frame_ids[1] >= 0:
                    offset = 0
                    cur_id = frame_ids[1]
                traj_id = list(range(cur_id - past_nums, cur_id + 1)) + list(range(cur_id + 1, cur_id + 12 + 1))
                coor = []
                mask = []
                heading = Quaternion(seq_annos[frame_ids[1]]['box_anno']['rotation']).yaw_pitch_roll[0]
                for idx in traj_id:
                    if idx < 0:
                        coor.append(seq_annos[0]['box_anno']['translation'][:2])
                        mask.append(0)
                    elif idx >= len(seq_annos):
                        coor.append(seq_annos[-1]['box_anno']['translation'][:2])
                        mask.append(0)
                    else:
                        xy = seq_annos[idx]['box_anno']['translation'][:2]
                        coor.append(xy)
                        mask.append(1)
                ...
                return frames, {"coor": coor, "mask": mask, 'rot': rotation, 'offset': offset}
            # ================================================================================
        return frames, 0
