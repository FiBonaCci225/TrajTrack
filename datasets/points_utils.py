...
def apply_augmentation(pc, box, wlh_factor=1.25):
    in_box_mask = nuscenes.utils.geometry_utils.points_in_box(box, pc.points[:3,:], wlh_factor=wlh_factor)
...
def flip_augmentation(prev_points, prev_box, this_points, this_box):
    flip_y = np.random.choice([True, False])
    if flip_y:
        prev_points[:, 1] = -prev_points[:, 1]
        this_points[:, 1] = -this_points[:, 1]

        prev_new_box = copy.deepcopy(prev_box)
        rot_mat = copy.deepcopy(prev_box.rotation_matrix)
        trans = copy.deepcopy(prev_box.center)
        prev_new_box.translate(-trans)
        prev_new_box.rotate(Quaternion(matrix=rot_mat.T))
        prev_new_box.rotate(Quaternion(matrix=rot_mat.T))
        trans[1] = -trans[1]
        prev_new_box.translate(trans)
        prev_box = prev_new_box

        this_new_box = copy.deepcopy(this_box)
        rot_mat = copy.deepcopy(this_box.rotation_matrix)
        trans = copy.deepcopy(this_box.center)
        this_new_box.translate(-trans)
        this_new_box.rotate(Quaternion(matrix=rot_mat.T))
        this_new_box.rotate(Quaternion(matrix=rot_mat.T))
        trans[1] = -trans[1]
        this_new_box.translate(trans)
        this_box = this_new_box
    return prev_points, prev_box, this_points, this_box
...
def crop_pc_axis_aligned_in_range(PC, point_cloud_range, return_mask=False):
    """
    crop the pc using the box in the axis-aligned manner
    """

    x_filt_max = PC.points[0, :] < point_cloud_range[3]
    x_filt_min = PC.points[0, :] > point_cloud_range[0]
    y_filt_max = PC.points[1, :] < point_cloud_range[4]
    y_filt_min = PC.points[1, :] > point_cloud_range[1]
    z_filt_max = PC.points[2, :] < point_cloud_range[5]
    z_filt_min = PC.points[2, :] > point_cloud_range[2]
...
def crop_pc_in_range_mf(pc, sample_bb, ref_o, point_cloud_range, offset=2, oriented=True):
    """
    generating the search area using the sample_bb
    ...
    if oriented:
        ...
        new_pc = crop_pc_axis_aligned_in_range(new_pc, point_cloud_range)
        ...
        new_pc = crop_pc_axis_aligned_in_range(pc, point_cloud_range)
